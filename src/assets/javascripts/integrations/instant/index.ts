...
function handle(
  ...
  // Skip language switcher links - these are handled by their own click
  // handler and must not trigger instant navigation.
  if (el.closest("[data-md-component=\"i18n\"]"))
    return EMPTY
...
/**
 * Update the language switcher active state after instant navigation.
 *
 * The header is not replaced by instant navigation, so the active marker on
 * the language switcher must be updated client-side to match the new page's
 * locale.
 */
function updateI18nSwitcher(): void {
  const locale = document.documentElement.lang
  if (!locale)
    return
  document.querySelectorAll("[data-md-component=\"i18n\"] .md-select__link")
    .forEach(link => {
      link.classList.remove("md-select__link--active")
      if (link.getAttribute("hreflang") === locale)
        link.classList.add("md-select__link--active")
    })
}
