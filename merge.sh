#!/usr/bin/env bash
# Merge script: Convert ProperDocs + Material into unified DocsForge package
set -euo pipefail

cd /home/qq/.openclaw/workspace/projects/docsforge

echo "=== Step 1: Fix all internal imports ==="
# Replace 'from properdocs' and 'import properdocs' with 'from docsforge' / 'import docsforge'
find docsforge -name "*.py" -exec sed -i \
    -e 's/from properdocs\b/from docsforge/g' \
    -e 's/import properdocs\b/import docsforge/g' \
    -e 's/properdocs\.__version__/docsforge.__version__/g' \
    -e 's/properdocs\.__main__/docsforge.__main__/g' \
    -e 's/logging.getLogger(.properdocs.)/logging.getLogger("docsforge")/g' \
    -e "s/logging.getLogger('properdocs')/logging.getLogger('docsforge')/g" \
    {} +

echo "=== Step 2: Fix Material internal imports ==="
# Material imports itself as 'material' - replace with 'docsforge'
find docsforge/plugins -name "*.py" -exec sed -i \
    -e 's/from material\b/from docsforge/g' \
    -e 's/import material\b/import docsforge/g' \
    {} +
...
find docsforge -name "*.py" -exec sed -i \
    -e 's/"material\.templates"/"docsforge.themes.material.templates"/g' \
    -e "s/'material\.templates'/'docsforge.themes.material.templates'/g" \
    {} +

echo "=== Step 3: Create unified __init__.py ==="
...
echo "=== Step 4: Fix __main__.py (CLI) ==="
cat > docsforge/__main__.py << 'EOF'
...
echo "=== Step 5: Fix remaining Material root files ==="
# Remove duplicate/unneeded Material root files that conflict with engine
rm -f docsforge/exceptions.py docsforge/localization.py docsforge/plugins.py docsforge/replacement.py docsforge/replacement_warning.py docsforge/theme.py 2>/dev/null || true

echo "=== Step 6: Create unified pyproject.toml ==="
...
[project]
name = "docsforge"
description = "Unified documentation engine with Material theme."
readme = "README.md"
license = "GPL-3.0-or-later"
authors = [
    {name = "DocsForge Contributors"},
]
classifiers = [
...
# Keep backward-compatible entrypoints for MkDocs/ProperDocs plugins
...
echo "=== Step 7: Fix template path references in plugins ==="
find docsforge/plugins -name "*.py" -exec sed -i \
    -e 's/"material\//"docsforge\/themes\/material\//g' \
    -e "s/'material\//'docsforge\/themes\/material\//g" \
    {} +
