...
export class InitWizard {
  static async run() {
    ...
    const themeColor = await vscode.window.showQuickPick(
      ['teal', 'indigo', 'blue', 'green', 'purple'],
      { placeHolder: 'Select theme color' }
    );

    const enableSearch = await vscode.window.showQuickPick(
      ['Yes', 'No'],
      { placeHolder: 'Enable search?' }
    );
