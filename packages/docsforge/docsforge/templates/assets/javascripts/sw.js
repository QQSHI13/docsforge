...
function urlToKey(url) {
  ...
  return href.slice(ORIGIN_BASE.length);
}
...
async function deleteOrphans(newFiles) {
  const cache = await caches.open(CACHE_NAME);
  ...
  for (const req of keys) {
    ...
    if (!key) continue;
    if (key === MANIFEST_URL || key === 'sw.js') continue;
    if (!Object.prototype.hasOwnProperty.call(newFiles, key)) {
