...
- **隐私模式支持按主机排除** —— `assets_include`/`assets_exclude`
  改为同时匹配规整后的 `host/path` 与裸主机名，`*.clouddn.com`
  即可排除整个主机，无需猜路径前缀。被排除的 URL
  保持远程链接，不下载。
...
- **失败的外部资源链接到真实 URL 并重试** —— 页面改写不再等待后台预
  热下载的*远程* URL（曾在 HTML 中产生 `/https://...` 垃圾链接并在本地
  服务时 404）；改写会联入运行中的任务、必要时回退同步下载，失败的 URL
  每次构建都会重试（无负缓存）。同时修复改写路径首次提交外部资源时的
  执行器构造死锁。
- **BCP 47 搜索语言映射为支持的语言** —— 每语言搜索索引用原始语言标
  签（`zh-tw`、`pt-BR`），Marz 以通用空白分词索引并告警。现映射到基础
  语言（`zh-tw` → `zh`），保留真实分词。

### 修复

- **搜索结果高亮完整查询词** —— 结果片段曾使用原始 Marz
  索引词（词干化后，如 `docsforge` → `docsforg`），导致只标出单词片段。
  现改为高亮命中的查询词，与 `?h=` 高亮机制一致。
- **演示站标签索引恢复渲染** —— `tags.md`
  曾使用标签插件无法匹配的 `docsforge/themes/material/tags`
  指令（`listings_directive` 为 `material/tags`），页面只剩过时的手写分
  节。现改用真实指令，并删除已被生成列表取代的手写分节。
- **自动导航大写罗马数字** —— `part-i`
  这类目录分节曾渲染为 “Part i”。独立罗马数字单词现大写（“Part I” … “Part V”）。
- **自动导航使用真实标题** —— 没有 `nav:`
  时，页面曾变成以原始文件名标记的空分节（`my-page.md`），并刷出简写
  弃用警告。自动导航现生成显式条目，页面标题按 frontmatter → H1 →
  文件名解析，目录成为名称规整的分节。演示站点去掉显式 nav，改走该路径。
- **Studio 更新流程先展示信息** —— 引擎与扩展同时有更新时，先用一个
  对话框列出两个版本及各自发行注记，再选择更新方式；方法选定后直接执
  行。发行注记在 VS Code 内置浏览器中打开（不可用时回退外部浏览器），
  扩展注记链接修正为真实 tag（预发布注记此前指向不存在的 tag）。
- **Studio 审计修复** —— 无关打开文件夹不再遮蔽已配置项目（含初始化向
  导）；取消构建静默结束而非误报失败；围栏与行内代码中的链接脚注不再产
  生诊断、跳转与改写；不再询问按通道新鲜度记录，自动检查仅对新鲜更新
  弹窗；离线提示取较旧通道时间；预览与文档链接共用内置浏览器回退；链接
  修复定位精确出现位置；解释器变化时图标缓存失效；坏掉的显式解释器每会
  话警告一次并共用同一校验器；注册 `pickLinkFix`；全文范围使用精确位
  置；停用时释放缓存。
- **更新流程只问一次** —— 记住的选择有效时直接复用；仅在没有有效记住项
  时弹窗（环境命令仍每次询问）。“不再询问”区分缓存与新鲜数据；基于缓
  存的“已是最新”会明确标注；显式配置的坏解释器每会话警告一次而非静默
  回退；删除无用的更新状态字段。

### 优化

- **Git 日期加速** —— 每页构建曾派生 3 个 git 进程（`rev-parse` +
  两次 `log`，含全历史 `--follow`），演示站全量构建约 12
  秒花在 git 上。现 `rev-parse` 只跑一次，一次 `--follow`
  同时取更新/创建日期（首行恒等于 `log -1`，已在全部文档/演示页验证），
  并在渲染前对待建页面并发预热（有界线程池，输出与逐页查询完全一
  致）。演示站全量构建：14.9 秒 → 11.9 秒。
- **语言文件名大小写不再导致构建崩溃** —— BCP 47
  标签大小写不敏感，但主题翻译导入曾是严格的
  `partials/languages/<locale>.html`：配置 `zh-tw`
  找不到自带的 `zh-TW.html`，构建直接中止。现改由
  Python 侧解析（精确匹配、再大小写无关匹配、再回退 `en`），手写渲染上
  下文兜底 `en`（搜索/博客翻译器显式传入主题语言）。
