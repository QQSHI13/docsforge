...
- **翻译页面的导航图标** —— 后缀式 i18n 站点中，带语言后缀的页面（如
  `index.zh.md`）不会被识别为索引页（`File.name` 保留语言后缀），导致所有非默认
  语言丢失通过 frontmatter `icon:` 配置的顶部标签图标，`navigation.indexes`
  的分区索引检测对翻译页也随之失效。现在带后缀的孪生页面与其基础页面一样
  视为索引页。
- **语言导航完整渲染** —— 语言导航的页面源文件此前按渲染顺序懒加载读取，
  先渲染的页面看到的后读页面的导航标题和 frontmatter 图标为空。现在语言导航
  源文件在首页渲染前预读。
