...
class BlogPlugin(BasePlugin[BlogConfig]):
    ...
    def on_page_deps(self, deps, /, *, page, files, config):
        """Declare render dependencies for blog views.

        The blog entrypoint (and archive/category/profile views) renders a
        listing of every post, so any change to the post set — added, removed,
        or edited — must re-render those pages even when their own source is
        unchanged. This keeps the entrypoint fresh on incremental builds.
        """
        if not self.config.enabled or getattr(self, "blog", None) is None:
            return deps
        if page is self.blog or isinstance(page, View):
            posts = [post.file.abs_src_path for post in self.blog.posts if post.file.abs_src_path]
            deps.extend(sorted(set(posts)))
            # Author metadata (names, avatars, descriptions) is rendered into
            # every view; re-render when it changes.
            if self.config.authors:
                docs = os.path.relpath(config.docs_dir)
                authors_path = os.path.join(
                    docs,
                    os.path.normpath(self.config.authors_file.format(blog = self.config.blog_dir)),
                )
