...
class CacheManager:
    ...
    def _write_text(self, path: Path, text: str) -> None:
        """Write a scalar text file atomically (tmp + os.replace, PID-unique)."""
        tmp = path.with_name(f"{path.name}.{os.getpid()}.tmp")
        key = str(path.resolve())
        lock = self._write_locks.get(key)
        if lock is None:
    ...
    def set_config_hash(self, hash: str) -> None:
        ...
        self._write_text(self.config_hash_file, hash)
    ...
    def set_version(self, version: int) -> None:
        ...
        self._write_text(self.version_file, str(version))
    ...
    def set_pkg_version(self, version: str) -> None:
        ...
        self._write_text(self.pkg_version_file, version)
    ...
    def set_theme_sig(self, sig: str) -> None:
        ...
        self._write_text(self.theme_sig_file, sig)
    ...
    def set_nav_sig(self, sig: str) -> None:
        ...
        self._write_text(self.nav_sig_file, sig)
...
class BuildPlanner:
    ...
    # Outputs generated by the build itself (theme templates, search index,
    # PWA manifest, service worker) have no docs_dir counterpart and must
    # never be treated as orphaned.
    GENERATED_OUTPUTS: ClassVar[frozenset[str]] = frozenset(
        {
            "sitemap.xml",
            "sitemap.xml.gz",
            "404.html",
    ...
    def find_orphaned_outputs(
        self,
        source_dir: Path,
        output_dir: Path,
        known_dest_uris: set[str] | None = None,
    ) -> list[Path]:
        ...
        for output_file in output_dir.rglob("*"):
            ...
            # Generated outputs (theme assets, search index, PWA files) have
            # no docs counterpart — never treat them as orphaned.
            if rel_unix in self.GENERATED_OUTPUTS or rel_unix.startswith("assets/"):
                continue
            # When the caller provides the expected output set, only outputs
            # outside that set can be orphaned.
            if known_dest_uris is not None and rel_unix in known_dest_uris:
                continue
    ...
    def update_cache(self, source: Path, output: Path, deps: list[str] | None = None) -> None:
        ...
        if deps is not None:
            old_deps = set(self.deps.get(str(source), []))
            new_deps = set(deps)
            # Prune hashes of deps that are no longer listed so stale entries
            # cannot force future rebuilds (or mask removed-dependency churn).
            for removed in old_deps - new_deps:
                self.hashes.pop(removed, None)
            if deps:
